...
public class Polygon2D implements Shape, Cloneable, Serializable {
    ...
    public boolean contains(ceptraj.tool.Point p){
        return contains(p.getLat(), p.getLon());
    }
    
    public boolean contains(ceptraj.tool.Point[] ps){

        if(ps != null){
            return contains(ps[0]) && contains(ps[ps.length-1]);
        }
        
        return false;
    }
    
    public boolean contains(Collection<ceptraj.tool.Point> ps){
        if(ps != null){
            return contains(ps.toArray(new ceptraj.tool.Point[ps.size()]));
        }
        return false;
    }

    public ceptraj.tool.Point getClosestPointToPoint(ceptraj.tool.Point point){
        ceptraj.tool.Point result = new ceptraj.tool.Point();
    ...
    public boolean isWithinDistance(ceptraj.tool.Point point, double distance){
        
        boolean result = false;
        
        if(!contains(point)){
            ceptraj.tool.Point closestPointInPolygon = getClosestPointToPoint(point);        
            result = LocationFunction.haversineDistance(point, closestPointInPolygon) < distance;
        }
        return result;
    }
    
    public boolean isWithinDistance(ceptraj.tool.Point[] points, double distance){        
        return isWithinDistance(points[0],distance) || isWithinDistance(points[points.length-1],distance);
    }
    
    public boolean isWithinDistance(Collection<ceptraj.tool.Point> points, double distance){        
        return isWithinDistance(points.toArray(new ceptraj.tool.Point[points.size()]), distance);
    }
